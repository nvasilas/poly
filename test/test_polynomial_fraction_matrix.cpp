...
static const PolynomialFraction<int> f1(p1, p2);
static const PolynomialFraction<int> f2(p3, p4);

TEST(TestPolynomialFractionMatrix, constructor_rows_cols_size)
{
    auto m1 =
        (PolynomialFractionMatrix<int>(2, 2) << f1, f2, f2, f1).finished();
    ASSERT_EQ(m1.rows(), 2);
    ASSERT_EQ(m1.cols(), 2);
    ASSERT_EQ(m1.size(), 4);

    auto m2 =
        (PolynomialFractionMatrix<int>(1, 4) << f1, f2, f2, f1).finished();
    ASSERT_EQ(m2.rows(), 1);
    ASSERT_EQ(m2.cols(), 4);
    ASSERT_EQ(m2.size(), 4);
}
...
TEST(TestPolynomialFractionMatrix, operator_not_equal)
{
    auto m1 =
        (PolynomialFractionMatrix<int>(2, 2) << f1, f2, f2, f1).finished();
    auto m2 =
        (PolynomialFractionMatrix<int>(2, 2) << f2, f2, f2, f1).finished();
    ASSERT_NE(m1, m2);
    ASSERT_NE(m2, m1);
}
...
{
    auto m = (PolynomialFractionMatrix<int>(2, 2) << f1, f2, f2, f1).finished();
    auto expected =
        (PolynomialFractionMatrix<int>(2, 2) << 2 * f1, 2 * f2, 2 * f2, 2 * f1)
            .finished();
...
TEST(TestPolynomialFractionMatrix, operator_multiply_2x2)
{
    auto m1 =
        (PolynomialFractionMatrix<int>(2, 2) << f1, f2, f2, f1).finished();
    auto m2 = m1;
    auto expected = (PolynomialFractionMatrix<int>(2, 2) << f1 * f1 + f2 * f2,
...
{
    auto m1 = (PolynomialFractionMatrix<int>(1, 2) << f1, f2).finished();
    auto m2 = (PolynomialFractionMatrix<int>(2, 1) << f2, f1).finished();
    auto expected =
        (PolynomialFractionMatrix<int>(1, 1) << f1 * f2 + f2 * f1).finished();
    ASSERT_EQ(m1 * m2, expected);
}
...
TEST(TestPolynomialFractionMatrix, get_left)
{
    ...
    using F = PolynomialFraction<int>;
    auto mf =
        (PolynomialFractionMatrix<int>(3, 2) << F{{1}, {2, 1}}, F{{0}, {1}},
         F{{1}, {3, 1}}, F{{1}, {-3, 1}}, F{{1}, {2, -3, 1}}, F{{0}, {1}})
            .finished();
    auto [Nl, Dl] = get_left(mf);

    auto expected_Nl = (PolynomialMatrix<int>(3, 2) << P{1}, P{0}, P{-3, 1},
                        P{3, 1}, P{1}, P{0})
                           .finished();

    auto expected_Dl = (PolynomialMatrix<int>(3, 3) << P{2, 1}, P{0}, P{0},
